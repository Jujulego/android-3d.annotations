...
import net.capellari.julien.kotlinwriter.lambda
...
abstract class AbsGenerator(processingEnv: ProcessingEnvironment) {
    ...
    protected fun genGeneratorType(gen: Generator) = lambda("" of Int::class, returns = gen.kcls)

    protected fun getInterface(gen: Generator, name: String): TypeName {
        if (name == "Matrix") {
            return ClassName("net.capellari.julien.threed.math", name)
                .parameterizedBy(gen.kcls.asTypeName(), gen.degree_cls, gen.degree_cls)
        }
